namespace :send do
  ...
  task :send_deily_job => :environment  do
    ...
    Resume.find_each do |resume|
      begin
        if resume.client.send_email
          jobs = Job.includes(:company,:location).search_for_send(value: resume.key, location:resume.location_id).to_a
    ...
    Clientforalert.find_each do |client|
      begin
        jobs = Job.includes(:company,:location).search_for_send(value: Search.str_to_search(client.key.delete("<>{}#@!,:*&()'`\"’|")), location:client.location_id).to_a
        if jobs.present? and jobs.count == 3
