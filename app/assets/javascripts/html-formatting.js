var
    htmlFormatting = (function () {
        ...
        var
            ...
            checkAttrs = function (node, valid_attributes) {
                var i, re;

                if (typeof valid_attributes === 'string' && node.attributes.length) {
                    for (i = node.attributes.length - 1; i >= 0; i--) {
                        re = new RegExp('(?:^|,)' + node.attributes[i].name + '(?:,|$)');

                        if (!re.test(valid_attributes)) {
                ...
                if (node.nodeType === 1) {
                    ...
                    if (rule) {
                        ...
                        if (rule.no_empty && isEmpty(node)) {
                            ...
                            checkAttrs(node, rule.valid_attributes);
