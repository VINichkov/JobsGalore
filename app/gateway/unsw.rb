      ...
      unless span.text.empty?
        ...
        if hash.class == Hash
          ...
          if hash["Posted Date"].strftime("%d/%m/%Y")== DateTime.now.strftime("%d/%m/%Y")
            hash[:title] = td.css('a')&.text.delete(/ - 60051/)
            ...
            hash[:description] = Markitdown.from_nokogiri(Nokogiri::HTML(description.gsub("<br>"," ").gsub("<h1>","<h4>").gsub("<h2>","<h4>").gsub("<h3>","<h4>").gsub("</h2>","</h4>").gsub("</h3>","</h4>").squish.gsub("> <","><")))
