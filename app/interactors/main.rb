class Main
  ...
  def call
    ...
    if context.query && context.query["location_id"].present?
      ...
      context.jobs = JobDecorator.decorate_collection(Job.select(:id, :title,:description, :location_id, :company_id, :updated_at).where.not(urgent: nil).includes(:location,:company).last(10).reverse)
      if context.jobs.count == 0
